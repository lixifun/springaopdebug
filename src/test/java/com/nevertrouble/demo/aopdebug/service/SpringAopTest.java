...
class SpringAopTest {
    ...
    static {
        ...
        if (version.startsWith("1.8") || version.compareTo("1.8") < 0) {
            // 1.8 及以下
    ...
    // 有接口但注入实现类，由于注入的是 JDK Proxy 的代理类，它是 PrintService 类型，但不是 PrintServiceImpl
    // 导致程序无法运行
    ...
    @Autowired
    private PrintServiceImpl printService; // 此情况下，注入的是 JDK Proxy 代理类

    // 注入无接口的类
    // 由于这个类没有接口，走的 CGlib 代理，生成的是 DirectPrintWithoutImpl 的子类，可以注入成功
    @Autowired
